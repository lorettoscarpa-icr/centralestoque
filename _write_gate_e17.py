"""
Grava appVerMin='e17' (fecha a sessão "voltar a projetar melhor" — 22/09/2026:
ARQUITETURA_SYNC.md documentado, badge de saúde no painel, bloco de arquitetura
em pe-core.js) nos 3 docs de fábrica em ls_pe (manutt, tarragona,
tchwm-industria-e-comercio-de-calcados-ltda) e no doc global ls_pe_config/loja
(fonte do overlay bloqueante de UI — ver index.html, bloco "GATE TOTAL DE VERSÃO").
...
Só roda depois de confirmar via curl que o GitHub Pages já serve app-ver e17
(ver ARQUITETURA_SYNC.md) — gravar appVerMin antes disso travaria quem ainda
...
def grava_fabrica(did):
    ...
    def tx_fn(tx):
        ...
        entry = {
            ...
            "tipo": "gate de versão e17 — arquitetura documentada + badge de saúde",
            "mudancas": [],
        }
        tx.set(
            ref,

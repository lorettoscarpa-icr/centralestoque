"""
Grava appVerMin='e18' (fecha o conserto do bug das etiquetas em PDF fatiando
em duas páginas na Manutt — 22/09/2026: falta page-break-inside:avoid/
break-inside:avoid no .etiqueta, e window.print() sem esperar document.fonts
carregar — ver RELATORIO_ETIQUETAS_22SET.md) nos 3 docs de fábrica em ls_pe
(manutt, tarragona, tchwm-industria-e-comercio-de-calcados-ltda) e no doc
global ls_pe_config/loja (fonte do overlay bloqueante de UI — ver index.html,
bloco "GATE TOTAL DE VERSÃO").
...
Só roda depois de confirmar via curl que o GitHub Pages já serve app-ver e18
— gravar appVerMin antes disso travaria quem ainda não tem como atualizar.
"""
import time
...
def grava_fabrica(did):
    ...
    def tx_fn(tx):
        ...
        entry = {
            ...
            "tipo": "gate de versão e18 — fix etiquetas em PDF fatiando (break-inside:avoid)",
            "mudancas": [],
        }
        tx.set(
            ref,
